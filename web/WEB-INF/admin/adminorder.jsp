...
<head>
    ...
    <fmt:message bundle="${locale}" key="error.message.not.send" var="error"/>
    <fmt:message bundle="${locale}" key="hint.user.name" var="name"/>
...
<table class="table" style="width: 100%; background: #f8f9fa">
    ...
    <tbody>
    <jsp:useBean id="ordersForAdmin" scope="session" type="java.util.Map"/>
    <c:forEach items="${ordersForAdmin}" var="order" varStatus="orderCount">
        <tr bgcolor="#f5f5f5">
            <td>${order.getKey().getId()}</td>
            <td>${order.getKey().getOrderDate()}</td>
            <td>${order.getKey().getUserId()}</td>
            <td>
            ...
            <td>
            <form class="form-group" action="SendOrder" method="post">
